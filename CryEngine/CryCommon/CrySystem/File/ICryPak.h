...
inline FILE* fxopen(const char* file, const char* mode, bool bGameRelativePath = false)
{
	...
	if (gEnv && gEnv->pCryPak)
	{
		...
		wchar_t adjustedPathEx[_MAX_PATH + 1];
		mbstowcs(adjustedPathEx, adjustedPath, strlen(adjustedPath) + 1);
		HANDLE winFile = CreateFile(adjustedPathEx, winAccessFlags, FILE_SHARE_READ, 0, winCreationMode, FILE_ATTRIBUTE_NORMAL, 0);
